...
class ReportsController extends Controller
{
	...
	public function getEmpAtten(Request $request) 
	{
		...
		if($id !== null AND $datefrom == null AND $dateto == null ) 
		{
		 	...
		 	$data = table::attendance()->join('tbl_people', 'tbl_people_attendance.reference','=','tbl_people.id')->where('tbl_people_attendance.idno', $id)->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours','status_timein','status_timeout','tbl_people.per_day')->get();
			return response()->json($data);
			...
			$data = table::attendance()->join('tbl_people', 'tbl_people_attendance.reference','=','tbl_people.id')->where('tbl_people_attendance.idno', $id)->whereBetween('date', [$datefrom, $dateto])->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours','status_timein','status_timeout','tbl_people.per_day')->get();
			return response()->json($data);
		} elseif ($id == null AND $datefrom !== null AND $dateto !== null) {
			$data = table::attendance()->join('tbl_people', 'tbl_people_attendance.reference','=','tbl_people.id')->where('tbl_people_attendance.idno', $id)->whereBetween('date', [$datefrom, $dateto])->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours','status_timein','status_timeout','tbl_people.per_day')->get();
