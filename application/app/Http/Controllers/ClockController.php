...
class ClockController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $authenticatedUserId = Auth::user()->idno;

        // Check if the provided ID matches the authenticated user's ID
        if ($request->idno !== $authenticatedUserId) {
            return response()->json([
                "error" => trans("You are only allowed to clock in for your own ID.")
            ]);
        }
        ...
        if ($type == 'timeout') 
        {
            ...
            if ($hasout != NULL) 
            {
                ...
                if($sched_out_time == NULL) 
                {
                    ...
                    $sched_clock_out_time_24h = Carbon::createFromFormat("h:i A", $sched_out_time);
                    $time_out_24h = Carbon::createFromFormat("Y-m-d H:i:s A", $timeOUT);
                    // print_r($time_out_24h.'---'.$sched_clock_out_time_24h);
                    // exit;
                    if ($sched_clock_out_time_24h->format('H:i') === '00:00') {
                        $sched_clock_out_time_24h->setTime(23, 59);
                    }
                    if ($time_out_24h->greaterThanOrEqualTo($sched_clock_out_time_24h)) {
                        $status_out = 'Late Out';
                    } else {
                        $status_out = ($time_out_24h->between(Carbon::createFromTime(0, 0, 0), $sched_clock_out_time_24h)) ? 'Early Out' : 'On Time';
                    }
